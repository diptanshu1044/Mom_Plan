...
import { X, Phone, Calendar, FileText, ChevronDown } from "lucide-react";
...
export function CaseDetailPanel({ caseId, onClose }: CaseDetailPanelProps) {
  ...
  const reminder = useMutation({
    ...
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ["partner-case-detail", caseId] }),
  });
          ...
          <>
            ...
            <div className="px-6 py-5 border-b border-surface-border flex items-start justify-between">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-x-3 gap-y-0.5 text-xs text-text-soft mt-1">
                    ...
                    {c.client_info.dob && (
                      <span>DOB {formatDate(c.client_info.dob)} {c.client_info.age != null && `(${c.client_info.age})`}</span>
                    )}
              ...
              <div className="flex items-center gap-2">
                ...
                <button onClick={onClose} className="p-2 rounded-lg hover:bg-primary-subtle text-text-soft">
                  <X className="w-5 h-5" />
                </button>
              </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-5">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="bg-primary-subtle/50 rounded-xl border border-surface-border p-4">
                  <h3 className="text-xs font-bold uppercase tracking-wider text-text-soft mb-3">Client Information</h3>
                  <dl className="space-y-2 text-sm">
                    {[
                      ["Phone", c.client_info.phone],
                      ["Email", c.client_info.email],
                      ["Address", c.client_info.address],
                      ["Household size", c.client_info.household_size ? `${c.client_info.household_size} (mother + children)` : null],
                      ["Children", c.client_info.children.map((ch) => `${ch.name} ${ch.age}`).join(", ") || null],
                ...
                <div className="bg-primary-subtle/50 rounded-xl border border-surface-border p-4">
                  <h3 className="text-xs font-bold uppercase tracking-wider text-text-soft mb-3">Eligibility Snapshot</h3>
                  <dl className="space-y-2 text-sm">
                    <div className="flex justify-between">
                      <dt className="text-text-soft">Monthly income</dt>
                      <dd className="font-medium">${c.eligibility.monthly_income?.toLocaleString() ?? "—"} (reported)</dd>
                    </div>
                    <div className="flex justify-between">
                    ...
                    <div className="flex justify-between">
                      ...
                      <dd>
                        {c.eligibility.last_verified ? formatDate(c.eligibility.last_verified, "MMM yyyy") : "—"}{" "}
                    ...
                    <div className="flex justify-between">
                      ...
                      <dd className={c.eligibility.proof_of_residency === "on_file" ? "text-status-success font-semibold" : "text-status-error font-semibold"}>
                    ...
                    {c.eligibility.postpartum_status && (
                      <div className="flex justify-between">
                        ...
                        <dd className="text-status-success font-semibold">{c.eligibility.postpartum_status}</dd>
                      </div>
                    )}
                    {c.eligibility.next_review_date && (
                      <div className="flex justify-between">
                        <dt className="text-text-soft">Next eligibility review</dt>
                        <dd className="text-status-error font-semibold">{formatDate(c.eligibility.next_review_date)}</dd>
                      </div>
                    )}
                  </dl>
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="bg-white rounded-xl border border-surface-border p-4">
                  <h3 className="text-xs font-bold uppercase tracking-wider text-text-soft mb-3">Documents</h3>
                      ...
                      <li key={doc.id} className="flex items-center justify-between gap-2 text-sm">
                        <div>
                          ...
                          <div className="text-xs text-text-soft">
                            {doc.status === "missing" ? "Not received" : `Received ${formatDate(doc.uploaded_at, "MMM d")}`}
                          </div>
                        </div>
                ...
                <div className="bg-white rounded-xl border border-surface-border p-4">
                  <h3 className="text-xs font-bold uppercase tracking-wider text-text-soft mb-3">Activity Timeline</h3>
                  <ul className="space-y-4">
                    {c.activity_log.map((act) => (
                      <li key={act.id} className="flex gap-3">
                        <div className={cn("w-2 h-2 rounded-full mt-1.5 shrink-0", ACTIVITY_COLORS[act.color] ?? "bg-text-soft")} />
                        <div>
